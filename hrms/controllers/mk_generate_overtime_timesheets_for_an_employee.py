...
@frappe.whitelist()
def generate_overtime_timesheets(employee_dn, start_date=None, end_date=None):
    logger.info(f"mk_generate_overtime_timesheets_for_an_employee.generate_overtime_timesheets.start: employee_dn: {employee_dn} start_date: {start_date} end_date: {end_date}")
    ...
    conditions = [attendance.docstatus == 1, attendance.status == "Present", employee.name == employee_dn,
    ...
    for entry in attendance_records:
        ...
        logger.info(f"mk_generate_overtime_timesheets_for_an_employee.generate_overtime_timesheets: holiday_dates: {holiday_dates}")
